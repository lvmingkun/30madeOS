...
struct TASK *task_init(struct MEMMAN *memman)
{
    ...
    task = task_alloc(); // 接收到alloc返回的第一个任务
    task->flags = 2; // 活动中的标志
    task->priority = 2; // 0.02 s
    task->level = 0; // 先将当前调用程序的任务暂时设置为 0，即最高
    task_add(task);
...
void task_run(struct TASK *task, int level, int priority)
{
    ...
    if(task->flags == 2 && task->level != level){ // 改变活动区中的level
        task_remove(task); // 这里执行后flag会变为 1，因此下面也会执行的
    }
    if(task->flags != 2){
        // 从休眠下唤醒的情况
        task->level = level;
...
void task_sleep(struct TASK *task)
{
    ...
    if(task->flags == 2){
        // 如果处于活动中
        now_task = task_now();
        task_remove(task); // 执行此语句flags变为 1
        if(task == now_task){
            // 如果是让自己休眠，则需要进行任务切换
            task_switchsub();
