...
; TAB=4
    ...
    CYLS EQU 10 ; 柱面数
    ORG 0x7c00 ; 启动程序的装载地址一般为 0x7c00 ~ 0x7dff
    ...
    ; 标准的FAT12格式软盘的必备专用的代码 Stand FAT12 format floppy code，即书写在开头的文件描述系统
		...
		DW 2880			    ; 该磁盘大小（必须为 2880 扇区 1440 * 1024 / 512）
		...
		DB "HARIBOTEOS "	; 磁盘的名称（必须为 11 字字节，不足填空格）
		DB "FAT12   "		; 磁盘格式名称（必须 8 字，不足填空格）
		RESB 18				; 先空出 18 字节
...
; 程序主主体
	...
	next:
		...
		MOV [0x0ff0], CH         ; 将 cyls 的值移动到 0x0ff0 当中，为了告诉sys磁盘装载内容的结束地址，因为cyls代表了 10 个柱面
        JMP 0xc200
...
; 程序启动后我们想要显示的内容
    putloop:
         mov al, [si]
         add si, 1   ; 给SI加 1, 循环打印
         cmp al, 0
         je fin  ; 若数据内存该处的值为零，则该程序终止
         mov ah,0x0e  ; 显示一个文字，调用 0x0e 号子中断程序
         mov bx, 15
         int 0x10 ; 设置中断程序, 10h 号中断代表显示字符串，调用bios的中断
         jmp putloop 
