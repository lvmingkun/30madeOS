...
void HariMain(void)
{
	...
	load_tr(3 * 8); // TR寄存器，记录当前运行哪一个任务，赋值时一定是GDT编号乘以 8
  	task_b_esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024 - 8; // 栈空间开始地址，栈的末尾地址
...
void task_b_main(struct SHEET *sht_back)
{
	...
	for(;;)
	{
		...
		if (fifo32_status(&fifo) == 0)
		{
			...
			{
				sprintf(s, "%11d", count);
				putfonts8_asc_sht(sht_back, 20, 144, COL8_FFFFFF, COL8_840084, s, 13);
				timer_settime(timer_put, 1);
			} else if (i == 100) {
				sprintf(s, "%11d", count - count0);
				putfonts8_asc_sht(sht_back, 20, 128, COL8_FFFFFF, COL8_840084, s, 13);
				count0 = count;
				timer_settime(timer_ls, 100);
			}
		}
