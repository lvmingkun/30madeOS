...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1)
{
	...
	for( h = h0; h <= h1; h++) // 从下往上, 所有图层都描绘一遍
	{
		...
		bx1 = vx1 - sht->vx0;
		by1 = vy1 - sht->vy0;
		if(bx0 < 0) {bx0 = 0;} // 这里的bx0、1 都是相对距离，即相对于sht->vx0，也就是刷新的部分 = 图层在画面上的的起始地址 + 相对于起始地址的距离
		if(by0 < 0) {by0 = 0;}
		if(bx1 > sht->bxsize) {bx1 = sht->bxsize;}
		if(by1 > sht->bysize) {by1 = sht->bysize;}
		if((sht->vx0 & 3) == 0){
			// 四字节型
			i = (bx0 + 3) / 4; // bx0除以 4（小数进位）
			i1 = bx1      / 4; // bx1除以 4（小数舍去)
			i1 = i1 - i;
			sid4 = sid | sid << 8 | sid << 16 | sid << 24;
			for (by = by0; by < by1; by++) {
				vy = sht->vy0 + by;
				for (bx = bx0; bx < bx1 && (bx & 3) != 0; bx++) { // 前面被 4 除以多余的部分逐字写入
					vx = sht->vx0 + bx;
				...
				for (bx += i1 * 4; bx < bx1; bx++) { /* 后面被 4 除以的多余部分逐字节写入 */
					vx = sht->vx0 + bx;
					if (map[vy * ctl->xsize + vx] == sid) {
						vram[vy * ctl->xsize + vx] = buf[by * sht->bxsize + bx];
					}
